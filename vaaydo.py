...
MATHEMATICAL ARSENAL (20 Engines):
 1. Black-Scholes-Merton Framework — Full pricing + 9 Greeks
 2. Multi-Estimator Volatility — Close-to-Close, Parkinson, Garman-Klass, Yang-Zhang
 3. GARCH(1,1) Volatility Forecasting — Conditional variance, persistence, half-life
 4. Volatility Risk Premium (VRP) — IV estimation from RV + adaptive premium
 5. Monte Carlo with Antithetic Variates — 10,000 effective paths, variance-reduced
 6. Kelly Criterion — Half-Kelly with confidence-weighted dynamic sizing
 7. Hidden Markov Model Regime Detection — Gaussian emissions, online Baum-Welch
 8. Kalman Filter — Adaptive signal smoothing + volatility estimation
 9. CUSUM Change Point Detection — Multi-stream structural break alerts
10. Bayesian Adaptive Thresholds — No-bias, data-driven boundary discovery
11. Higher-Order Greeks — Vanna, Volga, Charm, Speed
12. Composite Risk Score — Regime-dependent weighted risk aggregation
13. Ensemble POP — Multi-model fusion with inverse-variance weighting
14. Regime Transition Forecasting — Stability score + transition probability
15. Entropy-Based Uncertainty — Regime ambiguity quantification
16. Log-Normal Expected Moves — 1σ / 2σ / 3σ probability zones
17. Sharpe-Ratio Strategy Ranking — Risk-adjusted optimization
18. Information Ratio — Strategy performance benchmarking
19. Unified Conviction Scoring — §9.1 multi-factor weighted formula
20. Optimal DTE Selection — Theta decay curve analysis
...
st.set_page_config(
    page_title="VAAYDO | FnO Intelligence",
...
def get_fno_symbols():
    """Fetch F&O list from NSE with robust fallback"""
    try:
        s = requests.Session()
...
# ═══════════════════════════════════════════════════════════════════════════════
# L1: DATA INGESTION — Auto-fetch + Multi-Estimator Volatility (§3.2)
# ═══════════════════════════════════════════════════════════════════════════════

@st.cache_data(ttl=300, show_spinner=False)
def fetch_all_data(symbols_ns: list, days_back: int = 400):
    """
    Master data engine: OHLCV from yfinance → compute all analytics.
    Implements §3.2: Multi-Estimator Volatility (C2C, Parkinson, Garman-Klass, Yang-Zhang)
    Implements §3.3: Volatility Risk Premium IV estimation
    """
    ...
    for sym_ns in symbols_ns:
        ...
        try:
            ...
            yz_o = o_c_prev.rolling(20).var()
            yz_c = c_o.rolling(20).var()
            k = 0.34 / (1.34 + 21/19)
            yz_var = yz_o + k * yz_c + (1 - k) * gk_var.clip(lower=0)
            rv_yz = np.sqrt(yz_var.clip(lower=0)) * np.sqrt(252)

            # Weighted composite (empirical weights: YZ most accurate)
            ...
            # ── §3.3: IV Estimation with Volatility Risk Premium ──
            rv_history = rv_composite.dropna().values
            lookback = min(252, len(rv_history))
            if lookback > 20:
                ivp = float(np.sum(rv_history[-lookback:] <= current_rv) / lookback * 100)
            else:
                ivp = 50.0

            # VRP: IV typically trades at premium to RV (2-5% annualized for equity)
            # Adaptive VRP based on regime
            if ivp > 70:
                vrp_factor = 1.08   # high vol → VRP compresses
            elif ivp < 30:
                vrp_factor = 1.18   # low vol → VRP expands
            else:
                vrp_factor = 1.12   # normal VRP ~12%
            iv_estimate = current_rv * vrp_factor
            atmiv = iv_estimate * 100

            # ── §5.4: GARCH(1,1) Forecast ──
            ...
            lr_var = omega / max(1 - persistence, 0.001)
            half_life = -np.log(2) / np.log(persistence) if persistence < 1 else 999
            ...
            atr_val = float(tr.rolling(14).mean().iloc[-1]) if len(tr) >= 14 else price * 0.02
            ...
            # §5.4: CUSUM change detection
            cusum_pos, cusum_neg = 0.0, 0.0
            cusum_alert = False
            recent_lr = lr.values[-30:] if len(lr) >= 30 else lr.values
            mu = np.mean(recent_lr); sd = max(np.std(recent_lr), 1e-6)
            for r in recent_lr[-10:]:
                z = (r - mu) / sd
            ...
            results.append({
                ...
                'RV_C2C': round(float(rv_c2c.iloc[-1] * 100) if not np.isnan(rv_c2c.iloc[-1]) else 0, 2),
                'RV_Parkinson': round(float(rv_park.iloc[-1] * 100) if not np.isnan(rv_park.iloc[-1]) else 0, 2),
                'RV_GK': round(float(rv_gk.iloc[-1] * 100) if not np.isnan(rv_gk.iloc[-1]) else 0, 2),
                'RV_YZ': round(float(rv_yz.iloc[-1] * 100) if not np.isnan(rv_yz.iloc[-1]) else 0, 2),
            })
        except Exception:
            continue
...
# ═══════════════════════════════════════════════════════════════════════════════
# L4: BSM PRICING ENGINE — Full Greeks + Higher-Order (§6.1, §6.2)
# ═══════════════════════════════════════════════════════════════════════════════
...
class BSM:
    ...
    def greeks(cls, S, K, T, r, σ, otype='call'):
        """Full Greeks including higher-order: Vanna, Volga, Charm, Speed (§6.2)"""
        ...
        # Higher-order Greeks (§6.2)
        vanna = -nd1 * d2 / σ                           # dDelta/dσ
        volga = S * np.sqrt(T) * nd1 * d1 * d2 / σ     # d²V/dσ²
        charm_val = -nd1 * (2 * r * T - d2 * sT) / (2 * T * sT)  # dDelta/dT
        speed = -(γ / S) * (d1 / sT + 1)               # dGamma/dS

        return Greeks(delta=δ, gamma=γ, theta=θ, vega=ν, rho=ρ,
                     vanna=vanna, volga=volga, charm=charm_val, speed=speed)
    ...
    @classmethod
    def risk_score(cls, greeks_obj, iv, regime_vol_weight=1.0):
        """§6.3: Composite Risk Score with regime-dependent weights"""
        w = [0.25, 0.20, 0.25, 0.15, 0.15]  # delta, gamma, vega, theta_penalty, tail
        # In high-vol regimes, upweight vega
        w[2] *= regime_vol_weight
        total = sum(w)
        w = [x / total for x in w]
        delta_r = abs(greeks_obj.delta) * w[0]
        gamma_r = abs(greeks_obj.gamma) * iv * w[1] * 100
        vega_r = abs(greeks_obj.vega) / max(iv * 100, 1) * w[2]
        theta_p = max(-greeks_obj.theta, 0) / max(abs(greeks_obj.theta) + 1, 1) * w[3]
        tail = (abs(greeks_obj.vanna) + abs(greeks_obj.volga) * 0.1) * w[4]
        return (delta_r + gamma_r + vega_r + theta_p + tail) * 100


# ═══════════════════════════════════════════════════════════════════════════════
# L5: MONTE CARLO ENGINE — Antithetic Variates (§5.1)
# ═══════════════════════════════════════════════════════════════════════════════

class MC:
    @staticmethod
    def gbm_antithetic(S, σ, T, n=5000):
        """§5.1: GBM with antithetic variate sampling → 2n effective paths"""
        steps = max(int(T * 252), 1)
        dt = T / steps
        z = np.random.standard_normal((n, steps))
        z_full = np.vstack([z, -z])  # antithetic pairs → 2n paths
    ...
    def pop_strangle(S, cK, pK, prem, σ, T, n=5000):
        final = MC.gbm_antithetic(S, σ, T, n)[:, -1]
...
def regime_stability(ivp, rsi, garch_persist, cusum_alert):
    """§4.4: Regime Stability Score — likelihood current regime persists"""
    score = 0.5
    if 30 <= ivp <= 70: score += 0.15       # mid-range → stable
    if 40 <= rsi <= 60: score += 0.10        # neutral RSI → stable
    if garch_persist > 0.90: score += 0.10   # high persistence → stable vol
    if cusum_alert: score -= 0.25            # structural break → unstable
    return max(0.0, min(1.0, score))

def regime_vol_weight(vr):
    """§6.3: Regime-dependent weight multiplier for vega in risk score"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# L7: KELLY CRITERION (§7.1)
# ═══════════════════════════════════════════════════════════════════════════════

def kelly(p, w, l, confidence=1.0):
    """§7.1: Fractional Kelly with confidence-weighted dynamic sizing"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# L6: STRATEGY ENGINE — 10 strategies, BSM + MC + Ensemble POP (§9.2)
# ═══════════════════════════════════════════════════════════════════════════════

def snap(x, g): return round(x / g) * g

def ensemble_pop(pop_bsm, pop_mc, rmse_bsm=0.08, rmse_mc=0.05):
    """§9.2: Inverse-variance weighted multi-model fusion for POP"""
    w_bsm = 1.0 / (rmse_bsm**2)
    w_mc = 1.0 / (rmse_mc**2)
    return (w_bsm * pop_bsm + w_mc * pop_mc) / (w_bsm + w_mc)

def conviction_unified(ra, pop, ev, sharpe, stability, iv_norm):
    """§9.1: Unified Conviction = w_r*RA + w_p*POP + w_e*EV_norm + w_s*Sharpe_norm + w_st*Stability"""
    w = {'ra': 0.20, 'pop': 0.25, 'ev': 0.15, 'sharpe': 0.20, 'stab': 0.10, 'iv': 0.10}
    ev_norm = max(0, min(1, (ev + 50) / 100))  # normalize EV to 0-1
    sh_norm = max(0, min(1, (sharpe + 1) / 3))  # normalize sharpe to 0-1
    raw = (w['ra'] * ra + w['pop'] * pop + w['ev'] * ev_norm +
           w['sharpe'] * sh_norm + w['stab'] * stability + w['iv'] * iv_norm)
    return max(0, min(100, raw * 100))


def score_strategy(name, stock, settings):
    """Evaluate a single strategy with full BSM + MC + Ensemble + §9.1 Conviction"""
    ...
    try:
        if name == 'Short Strangle':
            ...
            pop_m, ev, std = MC.pop_strangle(S, cK, pK, nc, iv, T)
            pop_e = ensemble_pop(pop_b, pop_m)
            cg = BSM.greeks(S,cK,T,r,iv,'call'); pg = BSM.greeks(S,pK,T,r,iv,'put')
            ng = Greeks(delta=-(cg.delta+pg.delta), gamma=-(cg.gamma+pg.gamma), theta=-(cg.theta+pg.theta),
                       vega=-(cg.vega+pg.vega), vanna=-(cg.vanna+pg.vanna), volga=-(cg.volga+pg.volga))
            rs = BSM.risk_score(ng, iv, rvw)
            ra = 0.9 if vr in (VolRegime.HIGH, VolRegime.EXTREME) and tr==TrendRegime.NEUTRAL else 0.4
            sh = ev / std if std > 0 else 0
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[{'type':'Sell Call','strike':cK,'premium':cp},{'type':'Sell Put','strike':pK,'premium':pp}],
                max_profit=nc, max_loss=ml, breakeven_lower=pK-nc, breakeven_upper=cK+nc,
                ...
                stability_score=stab, width=cK-pK, net_credit=nc, risk_reward=nc/ml if ml>0 else 0, regime_alignment=ra)

        elif name == 'Short Straddle':
            K = snap(S, g)
            cp, pp = BSM.call(S,K,T,r,iv), BSM.put(S,K,T,r,iv)
            nc = cp + pp; ml = S * 0.10 * 0.5
            pop_b = 0.5 * (BSM.prob_otm(S,K+nc,T,iv,'call') + BSM.prob_otm(S,K-nc,T,iv,'put'))
            pop_m, ev, std = MC.pop_strangle(S, K, K, nc, iv, T)
            pop_e = ensemble_pop(pop_b, pop_m)
            cg = BSM.greeks(S,K,T,r,iv,'call'); pg = BSM.greeks(S,K,T,r,iv,'put')
            ng = Greeks(delta=-(cg.delta+pg.delta), gamma=-(cg.gamma+pg.gamma), theta=-(cg.theta+pg.theta),
                       vega=-(cg.vega+pg.vega), vanna=-(cg.vanna+pg.vanna), volga=-(cg.volga+pg.volga))
            rs = BSM.risk_score(ng, iv, rvw)
            ...
            sh = ev / std if std > 0 else 0
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[{'type':'Sell Call','strike':K,'premium':cp},{'type':'Sell Put','strike':K,'premium':pp}],
                max_profit=nc, max_loss=ml, breakeven_lower=K-nc, breakeven_upper=K+nc,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=ng,
                conviction_score=cv, optimal_dte=21 if ivp>70 else 30, risk_score=rs,
                stability_score=stab, net_credit=nc, risk_reward=nc/ml if ml>0 else 0, regime_alignment=ra)
            ...
            w = lc - sc; ml = max(w - nc, 0.01)
            pop_b = BSM.prob_otm(S,sc,T,iv,'call') * BSM.prob_otm(S,sp,T,iv,'put')
            pop_m, ev, std = MC.pop_ic(S, sc, lc, sp, lp, nc, iv, T)
            pop_e = ensemble_pop(pop_b, pop_m)
            ...
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Call','strike':sc,'premium':BSM.call(S,sc,T,r,iv)},
                    {'type':'Buy Call','strike':lc,'premium':-BSM.call(S,lc,T,r,iv)},
                    {'type':'Sell Put','strike':sp,'premium':BSM.put(S,sp,T,r,iv)},
                    {'type':'Buy Put','strike':lp,'premium':-BSM.put(S,lp,T,r,iv)}],
                max_profit=nc, max_loss=ml, breakeven_lower=sp-nc, breakeven_upper=sc+nc,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=ng,
                conviction_score=cv, optimal_dte=45, risk_score=rs, stability_score=stab,
                width=w, net_credit=nc, risk_reward=nc/ml, regime_alignment=ra)

        elif name == 'Bull Put Spread':
            sp_k, lp_k = snap(S-em*0.5, g), snap(S-em*1.2, g)
            nc = BSM.put(S,sp_k,T,r,iv) - BSM.put(S,lp_k,T,r,iv)
            ml = max((sp_k - lp_k) - nc, 0.01)
            pop_b = BSM.prob_otm(S, sp_k, T, iv, 'put')
            ev_c = pop_b * nc - (1-pop_b) * ml
            std = np.sqrt(pop_b*(nc-ev_c)**2 + (1-pop_b)*(-ml-ev_c)**2)
            pop_m = pop_b * 0.97  # MC approx
            pop_e = ensemble_pop(pop_b, pop_m)
            spg = BSM.greeks(S, sp_k, T, r, iv, 'put')
            ng = Greeks(delta=-spg.delta, theta=-spg.theta, vega=-spg.vega, vanna=-spg.vanna)
            rs = BSM.risk_score(ng, iv, rvw)
            ra = 0.8 if 'UP' in tr.value else 0.3
            sh = ev_c / std if std > 0 else 0
            cv = conviction_unified(ra, pop_e, ev_c, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Put','strike':sp_k,'premium':BSM.put(S,sp_k,T,r,iv)},
                    {'type':'Buy Put','strike':lp_k,'premium':-BSM.put(S,lp_k,T,r,iv)}],
                max_profit=nc, max_loss=ml, breakeven_lower=sp_k-nc, breakeven_upper=S*10,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev_c,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=ng,
                conviction_score=cv, optimal_dte=30, risk_score=rs, stability_score=stab,
                width=sp_k-lp_k, net_credit=nc, risk_reward=nc/ml, regime_alignment=ra)

        elif name == 'Bear Call Spread':
            sc_k, lc_k = snap(S+em*0.5, g), snap(S+em*1.2, g)
            nc = BSM.call(S,sc_k,T,r,iv) - BSM.call(S,lc_k,T,r,iv)
            ml = max((lc_k - sc_k) - nc, 0.01)
            ...
            ng = Greeks(delta=-scg.delta, theta=-scg.theta, vega=-scg.vega, vanna=-scg.vanna)
            rs = BSM.risk_score(ng, iv, rvw)
            ra = 0.8 if 'DOWN' in tr.value else 0.3
            sh = ev_c / std if std > 0 else 0
            cv = conviction_unified(ra, pop_e, ev_c, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Call','strike':sc_k,'premium':BSM.call(S,sc_k,T,r,iv)},
                    {'type':'Buy Call','strike':lc_k,'premium':-BSM.call(S,lc_k,T,r,iv)}],
                max_profit=nc, max_loss=ml, breakeven_lower=0, breakeven_upper=sc_k+nc,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev_c,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=ng,
                conviction_score=cv, optimal_dte=30, risk_score=rs, stability_score=stab,
                width=lc_k-sc_k, net_credit=nc, risk_reward=nc/ml, regime_alignment=ra)

        elif name == 'Iron Butterfly':
            K = snap(S, g); ww = snap(em*1.2, g)
            nc = (BSM.call(S,K,T,r,iv)+BSM.put(S,K,T,r,iv)) - (BSM.call(S,K+ww,T,r,iv)+BSM.put(S,K-ww,T,r,iv))
            ml = max(ww - nc, 0.01)
            pop_m, ev, std = MC.pop_ic(S, K, K+ww, K, K-ww, nc, iv, T)
            pop_b = 0.5 * (BSM.prob_otm(S,K+nc,T,iv,'call')+BSM.prob_otm(S,K-nc,T,iv,'put'))
            pop_e = ensemble_pop(pop_b, pop_m)
            ng = Greeks(theta=abs(BSM.greeks(S,K,T,r,iv,'call').theta)*2)
            rs = BSM.risk_score(ng, iv, rvw)
            ra = 0.7 if tr==TrendRegime.NEUTRAL else 0.3
            sh = ev / std if std > 0 else 0
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Call','strike':K,'premium':BSM.call(S,K,T,r,iv)},
                    {'type':'Sell Put','strike':K,'premium':BSM.put(S,K,T,r,iv)},
                    {'type':'Buy Call','strike':K+ww,'premium':-BSM.call(S,K+ww,T,r,iv)},
                    {'type':'Buy Put','strike':K-ww,'premium':-BSM.put(S,K-ww,T,r,iv)}],
                max_profit=nc, max_loss=ml, breakeven_lower=K-nc, breakeven_upper=K+nc,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=ng,
                conviction_score=cv, optimal_dte=30, risk_score=rs, stability_score=stab,
                width=ww, net_credit=nc, risk_reward=nc/ml, regime_alignment=ra)
            ...
            ml = max((sp_k - lp_k) - nc, 0.01)
            pop_b = 0.5*(BSM.prob_otm(S,cK,T,iv,'call')+BSM.prob_otm(S,sp_k,T,iv,'put'))
            pop_m = pop_b * 0.95; pop_e = ensemble_pop(pop_b, pop_m)
            ev = pop_e*nc - (1-pop_e)*ml
            std_e = max(abs(ev)*1.5, 0.01)
            ra = 0.7 if 'UP' in tr.value or tr==TrendRegime.NEUTRAL else 0.3
            sh = ev / std_e
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Call','strike':cK,'premium':BSM.call(S,cK,T,r,iv)},
                    {'type':'Sell Put','strike':sp_k,'premium':BSM.put(S,sp_k,T,r,iv)},
                    {'type':'Buy Put','strike':lp_k,'premium':-BSM.put(S,lp_k,T,r,iv)}],
                max_profit=nc, max_loss=ml, breakeven_lower=sp_k-nc, breakeven_upper=cK+nc,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,nc,ml,stab), net_greeks=Greeks(),
                conviction_score=cv, optimal_dte=35, risk_score=0, stability_score=stab,
                width=sp_k-lp_k, net_credit=nc, risk_reward=nc/ml, regime_alignment=ra)

        elif name == 'Calendar Spread':
            K = snap(S, g); fT, bT = T*0.5, T*1.5
            fp = BSM.call(S,K,fT,r,iv); bp = BSM.call(S,K,bT,r,iv*0.95)
            nd = bp - fp; mp = fp * 0.8; ml = max(nd, 0.01)
            pop_b = 0.55 if vr in (VolRegime.LOW, VolRegime.COMPRESSED) else 0.45
            pop_m = pop_b * 0.98; pop_e = ensemble_pop(pop_b, pop_m)
            ev = pop_e*mp - (1-pop_e)*ml
            ra = 0.8 if vr in (VolRegime.LOW,VolRegime.COMPRESSED) else 0.3
            sh = ev / max(abs(ev)*2, 0.01)
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Sell Call (Front)','strike':K,'premium':fp},
                    {'type':'Buy Call (Back)','strike':K,'premium':-bp}],
                max_profit=mp, max_loss=ml, breakeven_lower=K-mp, breakeven_upper=K+mp,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,mp,ml,stab), net_greeks=Greeks(theta=fp*0.03, vega=bp*0.01),
                conviction_score=cv, optimal_dte=45, risk_score=0, stability_score=stab,
                net_credit=-nd, risk_reward=mp/ml, regime_alignment=ra)

        elif name == 'Broken Wing Butterfly':
            c = snap(S, g); lo = snap(S-em*0.8, g); hi = snap(S+em*1.2, g)
            nc = abs(BSM.call(S,lo,T,r,iv) - 2*BSM.call(S,c,T,r,iv) + BSM.call(S,hi,T,r,iv)) * 0.1
            mp = (c-lo)+nc; ml = max((hi-c)-nc, 0.01)
            pop_b = 0.50; pop_m = 0.48; pop_e = ensemble_pop(pop_b, pop_m)
            ev = pop_e*mp*0.3 - (1-pop_e)*ml*0.3
            ra = 0.5
            sh = ev / max(abs(ev)*2, 0.01)
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Buy Call','strike':lo,'premium':-BSM.call(S,lo,T,r,iv)},
                    {'type':'Sell 2 Calls','strike':c,'premium':2*BSM.call(S,c,T,r,iv)},
                    {'type':'Buy Call','strike':hi,'premium':-BSM.call(S,hi,T,r,iv)}],
                max_profit=mp, max_loss=ml, breakeven_lower=lo, breakeven_upper=hi,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,mp*0.3,ml*0.3,stab), net_greeks=Greeks(),
                conviction_score=cv, optimal_dte=45, risk_score=0, stability_score=stab,
                width=hi-lo, net_credit=nc, risk_reward=mp/ml, regime_alignment=ra)

        elif name == 'Ratio Spread':
            lK = snap(S, g); sK = snap(S+em*0.8, g)
            lp_v = BSM.call(S,lK,T,r,iv); sp_v = BSM.call(S,sK,T,r,iv)
            mp = (sK-lK)+(2*sp_v-lp_v); ml = max(S*0.10, 0.01)
            pop_b = 0.55 if 'UP' in tr.value or tr==TrendRegime.NEUTRAL else 0.40
            pop_m = pop_b * 0.96; pop_e = ensemble_pop(pop_b, pop_m)
            ev = pop_e*mp*0.4 - (1-pop_e)*ml*0.2
            ra = 0.6 if 'UP' in tr.value else 0.3
            sh = ev / max(abs(ev)*2, 0.01)
            cv = conviction_unified(ra, pop_e, ev, sh, stab, iv_norm)
            return StrategyResult(name=name, legs=[
                    {'type':'Buy Call','strike':lK,'premium':-lp_v},
                    {'type':'Sell 2 Calls','strike':sK,'premium':2*sp_v}],
                max_profit=mp, max_loss=ml, breakeven_lower=lK, breakeven_upper=2*sK-lK+mp,
                pop_bsm=pop_b, pop_mc=pop_m, pop_ensemble=pop_e, expected_value=ev,
                sharpe_ratio=sh, kelly_fraction=kelly(pop_e,mp*0.4,ml*0.2,stab), net_greeks=Greeks(),
                conviction_score=cv, optimal_dte=30, risk_score=0, stability_score=stab,
                width=sK-lK, net_credit=2*sp_v-lp_v, risk_reward=mp/ml, regime_alignment=ra)
    except Exception:
        return None
    return None
...
def vol_estimator_chart(stock):
    """Multi-estimator volatility comparison bar chart (§3.2)"""
...
def main():
    ...
    with st.sidebar:
        st.markdown("""<div style="text-align:center; padding:1rem 0; margin-bottom:1rem;">
            ...
            <div style="color:#888; font-size:0.7rem; margin-top:0.25rem;">वायदो — The Promise</div>
        ...
        # Date calendar picker (user request #3)
        today = date.today()
        # Generate next 4 Thursday expiry dates (NSE weekly expiry)
        def next_expiries(from_date, count=8):
            expiries = []
            d = from_date
        ...
        expiry_dates = next_expiries(today)
        expiry_labels = [f"{d.strftime('%d %b %Y')} ({(d - today).days}D)" for d in expiry_dates]
        sel_idx = st.selectbox("Expiry Date", range(len(expiry_dates)), format_func=lambda i: expiry_labels[i], index=0)
        expiry_date = expiry_dates[sel_idx]
        dte = (expiry_date - today).days

        # Also allow custom date
        custom = st.checkbox("Custom date", value=False)
        if custom:
            custom_date = st.date_input("Select date", value=expiry_date, min_value=today + timedelta(days=1), max_value=today + timedelta(days=365))
            dte = (custom_date - today).days
        ...
        st.markdown(f"""<div class='ib'><p style='font-size:0.72rem; margin:0; color:#888; line-height:1.5;'>
            ...
            <strong>Conviction:</strong> §9.1 Unified Multi-Factor<br>
            <strong>Strategies:</strong> 10 Active</p></div>""", unsafe_allow_html=True)

    settings = {'dte': max(dte, 1), 'gap': strike_gap, 'capital': capital}

    # ── HEADER ──
    st.markdown(f"""<div class="hdr"><div class="badge">Hemrek Capital</div>
        <h1>VAAYDO — FnO Trade Intelligence</h1>
        <div class="tag">BSM · Monte Carlo (Antithetic) · Multi-Estimator Vol · GARCH · Kelly · Regime Intelligence &nbsp;|&nbsp;
    ...
    # ── COMPUTE STRATEGIES ──
    with st.spinner("Running BSM + Monte Carlo (antithetic, 10K paths) + Kelly analysis..."):
        all_trades = []
        for _, row in df.iterrows():
            ...
            if best:
                ...
                tr = detect_trend(rd['price'], rd.get('ma20_daily', rd['price']), rd.get('ma50_daily', rd['price']), rd.get('rsi_daily', 50), rd.get('% change', 0))
    ...
    with c5: st.markdown(f"<div class='mc {'bad' if cusum_alerts > 5 else 'warn' if cusum_alerts > 0 else 'ok'}'><h4>CUSUM Alerts</h4><h2>{cusum_alerts}</h2><div class='sub'>Structural breaks detected</div></div>", unsafe_allow_html=True)
    ...
    # ═══ TRADE RADAR ═══
    with tab1:
        st.markdown("<div style='margin-bottom:1rem;'><span style='font-size:1.1rem;font-weight:700;color:#EAEAEA;'>Top FnO Opportunities</span><span style='color:#888;font-size:0.85rem;margin-left:0.75rem;'>Ranked by §9.1 Unified Conviction · Ensemble POP · Antithetic MC</span></div>", unsafe_allow_html=True)
        top = filtered[:9]
        if not top:
    ...
    with tab2:
        ...
        if sel:
            ...
            stab = regime_stability(row['IVPercentile'], row.get('rsi_daily', 50), row.get('GARCH_Persistence', 0.95), row.get('CUSUM_Alert', False))
            ...
            # §3.2 Multi-Estimator Volatility Panel
            ...
            st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
            st.markdown("<span style='font-weight:700;color:#EAEAEA;'>Strategy Rankings</span><span style='color:#888;margin-left:0.75rem;font-size:0.85rem;'>All 10 strategies · §9.1 Unified Conviction · Ensemble POP</span>", unsafe_allow_html=True)
            ...
            for rank, s in enumerate(strats[:5], 1):
                ...
                with st.expander(f"{'🥇' if rank == 1 else '🥈' if rank == 2 else '🥉' if rank == 3 else f'#{rank}'} {s.name} — Conv: {cv:.0f} | POP: {s.pop_ensemble*100:.1f}% | Sharpe: {s.sharpe_ratio:.2f} | Risk: {s.risk_score:.0f}", expanded=(rank == 1)):
                    ...
                    with ec1:
                        lh = "<table class='gt'><tr><th>Leg</th><th>Strike</th><th>Premium</th></tr>"
                        for l in s.legs:
                    ...
                    with ec2:
                        ...
                        st.markdown(f"""<div class='ib'><h4>Analytics</h4><p>
                            ...
                            <strong>Stability:</strong> <span class='mono'>{s.stability_score:.2f}</span><br>
                            <strong>BEs:</strong> <span class='mono'>{fmt(s.breakeven_lower)} — {fmt(s.breakeven_upper)}</span></p></div>""", unsafe_allow_html=True)
                        g = s.net_greeks
                        st.markdown(f"""<div class='ib' style='margin-top:0.5rem;'><h4>Greeks (1st + 2nd Order)</h4><p>
                            <strong>Δ:</strong> <span class='mono'>{g.delta:+.4f}</span> &nbsp;
                            <strong>Γ:</strong> <span class='mono'>{g.gamma:+.6f}</span><br>
                            <strong>Θ:</strong> <span class='mono tg'>{g.theta:+.2f}/day</span> &nbsp;
                            <strong>ν:</strong> <span class='mono'>{g.vega:+.2f}</span><br>
                            <strong>Vanna:</strong> <span class='mono'>{g.vanna:+.4f}</span> &nbsp;
                            <strong>Volga:</strong> <span class='mono'>{g.volga:+.4f}</span><br>
                            <strong>Charm:</strong> <span class='mono'>{g.charm:+.6f}</span> &nbsp;
                            <strong>Speed:</strong> <span class='mono'>{g.speed:+.6f}</span></p></div>""", unsafe_allow_html=True)
    ...
    with tab3:
        if filtered:
            ...
            cols_show = ['Instrument', 'strategy', 'conviction_score', 'pop', 'ev', 'sharpe', 'kelly_frac', 'risk_score', 'stability',
                        'net_credit', 'max_profit', 'max_loss', 'IVPercentile', 'GARCH_Vol', 'price', 'vol_regime', 'trend_regime', 'CUSUM_Alert']
    ...
    with tab4:
        ...
        with lc1:
            st.markdown("<span style='font-weight:600;color:#EAEAEA;'>Monte Carlo Terminal Distribution (10K Antithetic Paths)</span>", unsafe_allow_html=True)
            np.random.seed(42); paths = MC.gbm_antithetic(lS, liv, lT, 5000); term = paths[:, -1]
            fig = go.Figure()
            ...
            st.markdown(f"""<div class='ib'><h4>MC Stats (10,000 antithetic paths)</h4><p>
